...
class SFTTrainer(Trainer):
    ...
    def __init__(
        ...
        #print("self.completion_only_loss", self.completion_only_loss)
        ...
        with PartialState().main_process_first():
            ...
            if not is_processed:
                ...
                if is_conversational(first_example):
                    ...
                    #print(next(iter(dataset)), "iter example")
                ...
                def mask_prompt_completion(input_ids, tokenizer):
                    # Convert the special string to token ids
                    generation_prefix = "<|im_start|>assistant\n"
                    prefix_ids = tokenizer(generation_prefix, add_special_tokens=add_special_tokens)["input_ids"]

                    # Find the start index of the assistant tokens
                    for i in range(len(input_ids) - len(prefix_ids) + 1):
                        if input_ids[i:i + len(prefix_ids)] == prefix_ids:
                            start_idx = i + len(prefix_ids)
                            break
                    else:
                        raise ValueError("Assistant prefix not found in input_ids")

                    completion_mask = [0] * start_idx + [1] * (len(input_ids) - start_idx)
                ...
                def tokenize(example, processing_class, dataset_text_field, add_special_tokens):
                    if "prompt" in example:  # prompt-completion case
                        ...
                        completion_mask = mask_prompt_completion(processed['input_ids'], processing_class)
                        processed["completion_mask"] = completion_mask
